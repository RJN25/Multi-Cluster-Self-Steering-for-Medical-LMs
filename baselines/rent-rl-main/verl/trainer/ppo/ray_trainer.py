...
class RayPPOTrainer:
    ...
    def _create_dataloader(self):
        ...
        print("target batch_size:", self.config.data.get("gen_batch_size", self.config.data.train_batch_size))
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            print("VAL TEST_BATCH LENGTH: ", len(test_batch))
            ...
            print("SIZE OF REWARD TENSOR:  ", len(reward_tensor))
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("old_log_prob", timing_raw):
                        ...
                        if self.config.trainer.minimize_entropy:
                            actual_reward_tensor, actual_reward_extra_infos_dict = compute_reward(batch, self.reward_fn)
                            reward_tensor = torch.zeros_like(batch.batch["responses"], dtype=torch.float32)
                            # Get the batch size and sequence length
                            batch_size, seq_length = entropys.shape
                                                      
                                
                            all_extracted_entropys = []
                            ...
                            for i in range(len(batch)):
                                ...
                                extracted_answer = self.extract_answer(data_source, response_str)
                                

                                # Extract entropies of all tokens
                                extracted_entropys = entropys[i, :valid_response_length].mean()
                                entropy_target = "".join(self.tokenizer.batch_decode(response_ids[:valid_response_length], skip_special_tokens=True))
                                
                                # Format reward
                                if self.config.trainer.base_format:
                                    if extracted_answer is None:
                                        extracted_entropys = torch.tensor(self.config.trainer.entropy_base)
                                    else:
                                        if self.config.trainer.set_entropy_zero:
                                            extracted_entropys = torch.zeros_like(extracted_entropys.mean())
                                        else:
                                            extracted_entropys = extracted_entropys.mean()
                                
                                reward_tensor[i, valid_response_length - 1] = extracted_entropys * -1.0
                                all_extracted_entropys.append(extracted_entropys)
                                all_extracted_answers.append(extracted_answer)
                                                                
                                all_entropy_targets.append(entropy_target)
                                reward_extra_infos_dict = defaultdict(list)
                            
                            batch.batch["extracted_entropys"] = all_extracted_entropys
                            batch.batch["extracted_answers"] = all_extracted_answers
                            batch.batch["entropy_targets"] = all_entropy_targets
                            batch.batch["actual_reward_tensor"] = actual_reward_tensor
                            if self.config.trainer.mask_invalid_loss:
                                batch.batch["loss_mask"] = torch.stack(loss_mask)
                ...
                if self.config.trainer.minimize_entropy:
                    metrics.update(compute_entropy_metrics(batch=batch, base_entropy=self.config.trainer.entropy_base))
                
                # visualize wandb table
                vis_batches = 3
                ...
                # TODO: fix table data to show visualizations for non-ent runs
                if self.config.trainer.minimize_entropy:
                    for i in range(len(prompts_text)):
                        ...
                        table_data = {
                            ...
                            "extracted_entropys": batch.batch["extracted_entropys"][i],
                            "extracted_answers": str(batch.batch["extracted_answers"][i]) if batch.batch["extracted_answers"][i] is not None else None,
                            "entropy_targets": batch.batch["entropy_targets"][i],
                            'reward': batch.batch["actual_reward_tensor"][i].sum(-1).item(),
                            'ground_truth': batch.non_tensor_batch["reward_model"][i].get("ground_truth", 'NaN'),
                        }
